...
class TwseSpider(scrapy.Spider):
    ...
    def __init__(self):
            ...
            if y ==  int(today.year):   #產生今年的年與月
                for m in range(1, int(today.month)+1):
                    if m < 10:
                        ...
                        s = str(y) + str(m) +'01'
                    dates.append(s)
        stockno_list = ['2330']    #要蒐集的股票代碼，可以是串列
        for stockno in stockno_list:
            for date in dates:
                ...
                self.start_urls.append(url)  #建立start_urls串列
    ...
    def parse(self, response):
        data_src = json.loads(response.body_as_unicode()) #證交所回傳資料為json，使用json.loads將json轉換成python的字典結構
        stockno = response.url[-4:]  #取出股票代碼
        item = TwseMongoItem()  #定義在items.py
